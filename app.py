...
def create_database():
    try:
        ...
        cur.execute("SELECT 1 FROM pg_catalog.pg_database WHERE datname = %s", (os.getenv('DB_NAME'),))
        exists = cur.fetchone()
...
def get_db_connection():
    try:
        ...
        print(f"Connection params: host={os.getenv('DB_HOST')}, user={os.getenv('DB_USER')}, db={os.getenv('DB_NAME')}")
        conn = psycopg2.connect(
            host=os.getenv('DB_HOST'),
...
def get_task(task_id):
    try:
        if task_id < 1:
            return jsonify({"error": "Invalid task ID. Task ID must be greater than 0"}), 400
...
def add_task():
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute("INSERT INTO tasks (task) VALUES (%s) RETURNING id, task;", (new_task,))
        task = cur.fetchone()
        conn.commit()
        cur.close()
        conn.close()
...
def update_task(task_id):
    try:
        ...
        cur.execute("UPDATE tasks SET task = %s WHERE id = %s RETURNING id, task;", (updated_task, task_id))
        task = cur.fetchone()
        conn.commit()
        cur.close()
        conn.close()
...
def delete_task(task_id):
    try:
        ...
        cur.execute("DELETE FROM tasks WHERE id = %s RETURNING id, task;", (task_id,))
        task = cur.fetchone()
        conn.commit()
        cur.close()
        conn.close()
